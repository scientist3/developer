...
if (isset($_POST['create'])) {
	$stmt = $pdo->prepare("INSERT INTO users1 (name, email) VALUES (?, ?)");
...
if (isset($_POST['update'])) {
	$stmt = $pdo->prepare("UPDATE users1 SET name = ?, email = ? WHERE id = ?");
	$stmt->execute([$_POST['name'], $_POST['email'], $_POST['id']]);
}

// Fetch all users
$users = $pdo->query("SELECT * FROM users1 ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
	<title>Simple CRUD with PHP & PostgreSQL</title>
</head>
